...
private:
    ...
    /**
     * @brief Engine of the VApp
     */
    MyEngine _engine;

    /**
     * @brief Add an action object to the VApp
     *
     * In the NXT Cockpit actions are displayed as buttons
     * An action object can be triggered via REST. (e.g. with CURL)
     * curl -u <CAMERA_USER>:<PW> -X POST -i <YOUR_CAMERA_IP>/vapps/vappbasic/actions/logmessageaction
     */
    std::shared_ptr<IDS::NXT::Action> _writeLogMessage;
